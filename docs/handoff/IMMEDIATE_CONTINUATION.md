# 立即接續 — CI83-canyon-relief-evidence

立即使用GitHub connector，必要時GoogleDrive connector。只讀STATUS與docs/evidence/CI83_CHECKPOINT.json、確認main一次後直接remainingReview。不是盤點、歷史審計或重規劃；不索取ROM/token/手動證據。
