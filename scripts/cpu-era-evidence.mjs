import {assertVisibilityStop} from './visibility-evidence.mjs';
...
export function inspectCpuEraEvidence({dir,buildDir,sourceSha,runId,runAttempt}) {
 ...
 for(const v of r.townReadability.stops){keep(v.image,true);const b=keep(v.canvasImage,true);need(b.readUInt32BE(16)===v.pixels.width&&b.readUInt32BE(20)===v.pixels.height,'town route canvas PNG dimensions');assertVisibilityStop(v,b);}
