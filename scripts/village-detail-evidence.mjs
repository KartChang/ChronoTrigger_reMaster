...
const need=(ok,why,observation)=>{if(!ok){const error=Error('Town detail evidence: '+why);if(observation)error.observation=structuredClone(observation);throw error;}};
...
export function assertTownDetails(v,renderSize){
 ...
 const s=v.sign;
 need(s?.name==='inn-sign'&&s.texture==='truce-inn-sign'&&s.vertices===4&&s.indices===6&&same(s.scaling,[2.75,2.75,1]),'original plane and declared scale');
 ...
 need(r&&['x','y','width','height'].every(k=>Number.isFinite(r[k]))&&r.width>=40&&r.height>=20&&Math.abs(r.width-2*r.height)<.001,'readable sign rectangle',{projection:p,renderSize,required:{width:40,height:20}});
