import {opaqueMipBlend} from './cpu-minification';
...
export class CpuRaster {
 ...
 private fill(av:ClipVertex,bv:ClipVertex,cv:ClipVertex,m:CpuSurface):void{
  ...
  let diffuse=m.texture,upper:CpuTexture|null=null,mipMix=0;
  if(diffuse?.mips?.length&&!m.textureAlpha&&!m.opacity&&!m.blend&&m.cutoff===0&&m.alpha===1&&a.a===1&&b.a===1&&c.a===1){
   const selection=opaqueMipBlend(a,b,c,diffuse.width,diffuse.height,diffuse.matrix,diffuse.mips.length);
   if(selection){
    const base=diffuse;
    diffuse=selection.lower===0?base:{...base,...base.mips![selection.lower-1]!};
    if(selection.mix>0){upper={...base,...base.mips![selection.upper-1]!};mipMix=selection.mix;this.fractionalTriangles++;}
