...
export type CpuTexture={width:number;height:number;rgba:Uint8ClampedArray;invertY:boolean;wrapU:number;wrapV:number;matrix:readonly number[]};
...
export class CpuRaster {
 ...
 private fill(av:ClipVertex,bv:ClipVertex,cv:ClipVertex,m:CpuSurface):void{
  ...
  for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){
   ...
   if(m.texture){const t=m.texture,i=textureIndex(t,u,v);r*=t.rgba[i]!/255;g*=t.rgba[i+1]!/255;blue*=t.rgba[i+2]!/255;if(m.textureAlpha)alpha*=t.rgba[i+3]!/255;}
