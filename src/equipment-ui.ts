...
export function equipmentPanel(root:HTMLElement,getState:()=>State,changed:(message:string)=>void,canAct:()=>boolean){
 ...
 function refresh(force=false):void{
  ...
  const focus=document.activeElement?.id,hadFocus=root.contains(document.activeElement);root.replaceChildren();
  ...
  const actors=document.createElement('div');actors.className='equipment-actions';root.append(actors);
  for(const m of present)addButton(actors,MEMBER_NAMES[m],()=>{selected=m;refresh(true);},m===selected,`equipment-member-${m}`);
  const bonuses=equipmentBonuses(e,selected),status=document.createElement('p');status.id='equipment-stats';status.textContent=`${MEMBER_NAMES[selected]}｜HP ${actor.hp}/${guest?GUEST_HP:MAX_HP} · MP ${actor.mp}/${guest?GUEST_MP:MAX_MP}\n普通攻擊加成 +${bonuses.attack}｜每次受擊減傷 ${bonuses.defense}（至少受 1 傷害）；技能與合技消耗不變。`;root.append(status);
  const modifiable=s.mode==='explore'&&!cutsceneActive(s);
  for(const slot of GEAR_SLOTS){
   const row=document.createElement('div');row.className='equipment-slot';const label=document.createElement('strong');label.textContent=`${slot==='weapon'?'武器':slot==='body'?'身體':'頭部'}：${itemById(e.worn[selected][slot])!.name}`;row.append(label);
  ...
  if(!shopAvailable(s)){const p=document.createElement('p');p.textContent='到千年祭南側，靠近梅爾基歐，再開啟背包買賣。戰鬥、演出、選擇途中不能交易。';shop.append(p);}
   ...
   const row=document.createElement('div');row.className='equipment-shop-row';const p=document.createElement('span');p.textContent=`${item.name}｜持有 ${e.owned[item.id]??0}（閒置 ${freeCopies(e,item.id)}）`;row.append(p);
  ...
  if(focus){const found=document.getElementById(focus);if(found instanceof HTMLButtonElement&&!found.disabled)found.focus();else if(hadFocus)document.getElementById('inventory-close')?.focus();}
 }
