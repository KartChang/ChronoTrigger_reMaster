 ...
 function refresh(force=false):void{
  ...
  for(const slot of GEAR_SLOTS){
   ...
   for(const item of EQUIPMENT.filter(i=>i.slot===slot&&i.members.includes(selected)&&((e.owned[i.id]??0)>0))){
    ...
    const b=addButton(row,(wearing?'已裝備 ':'換上 ')+item.name,()=>{const ok=equipItem(getState(),selected,item.id);changed(ok?`已換上${item.name}。`:'此時不能更換裝備。');refresh(true);},!modifiable||wearing||freeCopies(e,item.id)<1,`equip-${selected}-${item.id}`);b.dataset.equipmentItem=item.id;b.dataset.member=selected;b.dataset.current=String(wearing);
    const comparison=equipmentComparison(e,selected,item);b.setAttribute('aria-label',`${wearing?'已裝備':'換上'}${item.name}。${comparison}`);
    explain(b,wearing?`普攻 +${item.attack} · 減傷 ${item.defense}`:comparison);
    if(!modifiable&&!wearing)explain(b,'戰鬥／演出中不能換裝');else if(!wearing&&freeCopies(e,item.id)<1)explain(b,'其他隊員正在裝備');
   }root.append(row);
  }
  ...
  if(!shopAvailable(s)){const p=document.createElement('p');p.textContent='到千年祭南側，靠近梅爾基歐，再開啟背包買賣。戰鬥、演出、選擇途中不能交易。';shop.append(p);}
   ...
   if(buy.disabled)explain(buy,e.gold<item.price?'金幣不足':'持有已滿');
   if(sell.disabled)explain(sell,(e.owned[item.id]??0)>0?'裝備中，無閒置':'沒有可售物品');
   shop.append(row);
  }
