...
export function buildFairConduct(scene:Scene,parent:TransformNode){
 ...
 sprite('fair-cat-owner',girl.x,girl.z,1.12,1.55,WITNESS_SIZE.w,c=>drawWitness(c,'girl'));
 sprite('fair-lunch-owner',lunch.x-1.5,lunch.z,1.36,1.85,WITNESS_SIZE.w,c=>drawWitness(c,'elder'));
 sprite('fair-melchior',merchant.x,merchant.z,1.36,1.85,WITNESS_SIZE.w,c=>drawWitness(c,'merchant'));
 ...
 return {draw(s:State){
  const c=s.prologue.conduct,on=conductActive(s);root.setEnabled(on);if(!on||!c)return;
