...
type BoundaryState = Pick<State, 'chapter' | 'opening' | 'kingdom' | 'rescue'> & Partial<Pick<State,'prologue'|'trial'>>;
const key = (state: BoundaryState): string =>
  `${state.trial?.stage??'none'}/${state.trial?.fade?'fade':'stable'}/${state.trial?.luccaJoined??false}/${state.chapter}/${state.opening.phase}/${state.kingdom.phase}/${state.rescue.stage}/${state.prologue?.stage??'legacy'}/${state.prologue?.transition?'transition':'stable'}`;
