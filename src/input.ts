...
export type Command='attack'|'skill'|'combo'|'interact'|'pause'|'join';
export class Controls {
  private keys=new Set<string>();
  private touch=new Map<number,string>();
  private previous=new Map<string,boolean>();
  private bindings: Record<string,[Slot,Command]>={KeyJ:[0,'attack'],KeyK:[0,'skill'],KeyL:[0,'combo'],KeyE:[0,'interact'],Comma:[1,'attack'],Period:[1,'skill'],Slash:[1,'combo'],Numpad1:[1,'attack'],Numpad2:[1,'skill'],Numpad3:[1,'combo'],Enter:[1,'interact'],Escape:[0,'pause'],KeyC:[1,'join']};
  ...
  poll():Input {
    ...
    ([0,1] as Slot[]).forEach(slot=>{
      ...
      const mapping:[number,Command][]=[[0,'attack'],[2,'skill'],[3,'combo'],[1,'interact'],[9,slot===1?'join':'pause']];
