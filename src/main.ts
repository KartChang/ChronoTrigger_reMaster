...
async function loadGame():Promise<void>{if(state.mode!=='explore'||cutsceneActive(state)){announce('請先結束戰鬥。');return;}try{const raw=await storage.load(state.chapter);if(raw===null){announce('尚無本機存檔。');return;}state=deserialize(raw);controls.clear();updateHud();announce('讀檔完成。');}catch(e){announce('讀檔未完成：'+asError(e));}}
