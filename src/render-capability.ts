...
export type BackendHint='software'|'unverified';
...
export class RenderPolicy {
 private mode:RenderMode='auto';private budget=new RenderBudget();
 constructor(readonly hint:BackendHint){}
 setMode(value:unknown){this.mode=renderMode(value);this.budget.reset();}
 sample(ms:number,active:boolean){return this.budget.sample(ms,active&&this.mode==='auto');}
 scale(width:number,height:number,dpr:number){return renderScale(this.mode,this.hint,this.budget.inspect().level,width,height,dpr);}
 inspect(){const b=this.budget.inspect();return {profile:RENDER_PROFILE,mode:this.mode,backendHint:this.hint,
  reason:this.mode==='quality'?'user-quality':this.mode==='compatibility'?'user-compatibility':b.level?'sustained-frame-budget':this.hint==='software'?'software-driver-hint':'browser-default',
