...
export function buildTrial(scene:Scene,shadow:ShadowGenerator){
 type View={root:TransformNode;guards:Mesh[];enemies:Mesh[];tank:Mesh[];jurors:Mesh[];tankFrame:number;gate?:Mesh;fritz?:Mesh;marle?:Mesh;lid?:Mesh};
 const views=new Map<TrialMap,View>();
 function build(map:TrialMap):View{
  ...
  const picture=(n:string,x:number,z:number,w:number,h:number,draw:(c:CanvasRenderingContext2D)=>void,tw=24,th=32,y=h/2+.08,billboard=true)=>{
   ...
   const b=MeshBuilder.CreatePlane(map+n,{width:w,height:h},scene);b.parent=root;b.position.set(x,y,z);b.material=m;if(billboard)b.billboardMode=Mesh.BILLBOARDMODE_ALL;return b;
  };
  const v:View={root,guards:[],enemies:[],tank:[],jurors:[],tankFrame:-1};
  ...
  const guard=(x:number,z:number)=>picture('guard',x,z,1.2,1.7,ctx=>drawResident(ctx,'guard'));
  if(map==='courtroom'){
   for(let i=0;i<3;i++){
   ...
   picture('royal-stained-glass',0,6.65,7.2,6.8,drawCourtWindow,64,80,4.4,false);
   for(const side of [-1,1])for(let i=0;i<4;i++){box('velvet-curtain',side*(4.3+i*.26),3.1,6.45,.34,5.5-i*.35,.16,i%2?'#7d393c':'#9b4b46');box('curtain-gold-edge',side*(4.3+i*.26),.42+i*.18,6.39,.35,.09,.1,'#c9ac65');}
   box('judge-rostrum',0,1.02,4.2,1.7,.85,1.2,'#9e8c56');picture('judge',0,4.9,1.2,1.7,ctx=>drawResident(ctx,'king'),24,32,1.9);
   picture('defender',-3.1,3.4,1.1,1.7,ctx=>drawResident(ctx,'resident'));picture('prosecutor',3.1,3.4,1.1,1.7,ctx=>drawRescueNpc(ctx,'chancellor'));
   box('defendant-stand',0,.43,-1.2,1.5,.7,.7,'#a3854b');
   for(let i=0;i<7;i++){const side=i<4?-1:1,z=1+(i%4)*1.28;box('jury-tier',side*5.5,.3,z,2.3,.5,1.2,'#817c64');v.jurors.push(picture('juror-'+i,side*5.4,z,1,1.4,ctx=>drawResident(ctx,'resident'),24,32,1.05));}
   ...
   v.fritz=picture('fritz',2.1,2,1.2,1.7,ctx=>drawResident(ctx,'resident'));door(0,-6.8);
 ...
 return {inspect:()=>({built:[...views.keys()],visible:[...views.entries()].filter(([,v])=>v.root.isEnabled()).map(([k])=>k),assetProfile:ART_PROFILE.id,tankFrame:views.get('prisonbridge')?.tankFrame??null}),draw(s:State){
