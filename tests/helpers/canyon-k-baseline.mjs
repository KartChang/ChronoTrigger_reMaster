import {canyonLIfDeclared} from './canyon-l-baseline.mjs';
...
export function canyonKBaseline(name,source){
 source=canyonLIfDeclared(name,source);
...
export function canyonKIfDeclared(name,source){source=canyonLIfDeclared(name,source);const edits=canyonKSpec.files[name];return edits?.some(e=>source.includes(e.after))?canyonKBaseline(name,source):source;}
