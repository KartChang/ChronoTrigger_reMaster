import {canyonKIfDeclared} from './canyon-k-baseline.mjs';
...
export function detailJBaseline(name,source){
 source=canyonKIfDeclared(name,source);
...
export function detailJIfDeclared(name,source){source=canyonKIfDeclared(name,source);const edits=spec.files[name];return edits?.some(e=>source.includes(e.after))?detailJBaseline(name,source):source;}
