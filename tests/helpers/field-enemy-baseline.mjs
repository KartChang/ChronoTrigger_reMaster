import {visibilityIfDeclared} from './visibility-baseline.mjs';
...
export function fieldEnemyBaseline(name,source){
 source=visibilityIfDeclared(name,source);
...
export const fieldEnemyIfDeclared=(name,source)=>{source=visibilityIfDeclared(name,source);return Object.hasOwn(spec.files,name)&&spec.files[name].some(e=>source.includes(e.after))?fieldEnemyBaseline(name,source):source;};
