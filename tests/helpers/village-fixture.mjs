import {townCameraFixture} from './town-camera-fixture.mjs';
...
export function villageLayoutFixture(state,observation){
 return {beforeCamera:townCameraFixture({width:480,height:320}),afterCamera:townCameraFixture({width:480,height:320}),schema:'chrono-village-layouts-v1',status:'passed',physicalDevice:false,artApproved:false,before:structuredClone(state),after:structuredClone(state),fullStateEqual:true,restoredViewport:{width:960,height:640},views:[[960,640],[390,844],[844,390]].map(([width,height],i)=>{const value=observation('village-layout-'+i,'truce'),canvas={width:Math.floor(width/2),height:Math.floor(height/2)};
  ...
  return {...value,camera:townCameraFixture(canvas,state.ticks,value.village.details.sign.projection),paused:true,viewport:{width,height},pauseAccess:pauseAccessFixture(state,{width,height},i,observation),state:structuredClone(state),canvasImage:{source:'actual-cpu-canvas',path:`village-canvas-${i}.png`,bytes:99,sha256:'a'.repeat(64)}};})};
}
