...
import {townCameraFixture} from './helpers/town-camera-fixture.mjs';
...
function fixture(){
 ...
 return {before:{joined:false},beforeCamera,afterCamera:structuredClone(beforeCamera),views:[[960,640],[390,844],[844,390]].map(([w,h])=>{
  ...
  return {renderer,state:{ticks:7},village:{details},camera:townCameraFixture(renderer,7,details.sign.projection)};
 })};
}
...
for(const [name,mutate] of [
 ...
 ['lost second owner',f=>{f.before.joined=true;f.views[1].camera.rects=f.views[1].camera.rects.filter(x=>x.id!=='p1');f.views[1].camera.camera.actors=f.views[1].camera.camera.actors.filter(x=>x.id!=='p1');}],
...
])test('X evidence rejects '+name,()=>{const f=fixture();mutate(f);const before=JSON.stringify(f);assert.throws(()=>assertTownCameraLayouts(f),/Town camera evidence/);assert.equal(JSON.stringify(f),before);});
